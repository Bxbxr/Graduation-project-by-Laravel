        ...
        <div class="row">
            <div class="mx-auto col-9">
                ...
                <select id='qualityPick'>
                    <option value="1080" {{ $video->quality == 1080 ? 'selected' : '' }} {{-- {{ $video->quality < 1080 ? 'hidden' : '' }} --}}>1080p
                    </option>
                    <option value="720" {{ $video->quality == 720 ? 'selected' : '' }} {{-- {{ $video->quality < 720 ? 'hidden' : '' }} --}}>720p
                    </option>
                    <option value="480" {{ $video->quality == 480 ? 'selected' : '' }} {{-- {{ $video->quality < 480 ? 'hidden' : '' }} --}}>480p
                    </option>
                    <option value="360" {{ $video->quality == 360 ? 'selected' : '' }} {{-- {{ $video->quality < 360 ? 'hidden' : '' }} --}}>360p
                    </option>
                ...
                <div class="interaction text-left mt-2 like_area">
                    ...
                    @foreach ($video->views as $view)
                        <span class="float-right mt-2">عدد المشاهدات <span
                                class="viewsNumber">{{ $view->views_number }}</span></span>
                    @endforeach
                    <div class="loginAlert mt-3">
                ...
                <div class="mt-4 px-2">
                    <div class="comments">
                        ...
                        <div>
                            <textarea name="comment" id="comment" rows="4" class="form-control" placeholder="اضف تعليق عام"></textarea>
                            <button type="submit" class="btn btn-info mt-3 saveComment ">تعليق</button>
                            <div class="commentAlert mt-3 col-9 text-center">

                            </div>
                            <div class="commentBody rounded-lg">
                                                        ...
                                                        @if ($comment->user_id == auth()->user()->id || auth()->user()->administration_level > 1)
                                                            @if (!auth()->user()->block)
                                                                <form action="{{ route('comment.destroy', $comment->id) }}"
                                                                    method="POST"
                                                                    onsubmit="return confirm('هل انت نتأكد من انك تريد حذف هذا التعليق؟')">
                                                                    @csrf
                                                                    @method('DELETE')
